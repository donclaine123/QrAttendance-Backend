...
// Get teacher classes endpoint
router.get('/teacher-classes/:teacherId', async (req, res) => {
    const teacherId = req.params.teacherId;
    
    // Ensure authenticated user matches the requested teacher ID
    // or verify authentication explicitly
    if (!req.session || !req.session.userId || req.session.userId != teacherId) {
        console.log(`⚠️ Unauthorized access attempt to teacher classes. Session user: ${req.session?.userId}, Requested: ${teacherId}`);
        console.log(`⚠️ Session info:`, req.session || 'No session');
        console.log(`⚠️ Session ID:`, req.sessionID);
        console.log(`⚠️ Cookies:`, req.headers.cookie || 'No cookies');
        
        // Try to recover session from database directly
        try {
            if (req.sessionID) {
                ...
                if (sessions.length > 0 && sessions[0].user_id == teacherId) {
                    console.log(`✅ Session recovered from database for teacher ${teacherId}`);
                    // Continue processing without returning 401
                } else {
                    return res.status(401).json({ 
                        success: false, 
                        message: "Unauthorized: Authentication required or session invalid",
                        code: "AUTH_REQUIRED"
                    });
                }
            } else {
    ...
    try {
        const [classes] = await db.query(
            `SELECT * FROM class_records WHERE teacher_id = ? AND is_active = TRUE`,
            [teacherId]
        );
